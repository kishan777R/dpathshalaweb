...
$_SESSION['order_no'] = $_SESSION[$oid]['order_no'];
$_SESSION['totalamounttopay'] = $_SESSION[$oid]['totalamounttopay'];
$_SESSION['cartArr'] = $_SESSION[$oid]['cartArr'];
$_SESSION['studentdata'] = $_SESSION[$oid]['studentdata'];
$_SESSION['login_id'] = $_SESSION[$oid]['login_id'];
//echo "<pre>";
...
$posL = strpos($actual_link, "localhost");
if ($posL === false) {
} else {
...
$payment_request_id = $_GET['razorpay_payment_id'];
$payment_id = $_GET['razorpay_order_id'];
...
<script>
	...
	app.controller('myCtrl', ['$scope', '$http', function ($scope, $http) {
		...
		$scope.updateOrderStatus = function (c_id_int, bankArr, order_no, cart_array, totalamount, payment_status, payment_id) {
			...
			if (localStorage.getItem("add_aff_in_order")) {
				if (localStorage.getItem("add_aff_in_order") == 'YES') {
			...
			$http({
				...
				data: 'bankArr=' + encodeURIComponent(angular.toJson(bankArr)) + "&cart_array=" + encodeURIComponent(angular.toJson(cart_array)) + "&his_affilitater_c_id_int=" + his_affilitater_c_id_int + "&c_id_int=" + c_id_int + "&totalamount=" + totalamount + "&order_no=" + order_no,
				headers: {
				...
				if (jsondata.data.status == 'SUCCESS') {
					localStorage.removeItem("add_aff_in_order");
			...
			$http({
				...
				if ($scope.payment_status == 'Credit') {
					...
					setTimeout(function () {
						...
						if ($scope.payment_status == 'Credit') {
							window.location.href = "transaction-success?o=" + $scope.order_no;
						} else {
							window.location.href = "transaction-fail?o=" + $scope.order_no;
						}
					}, 1000);
				}
